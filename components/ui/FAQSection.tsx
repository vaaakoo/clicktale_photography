...
const FAQItemComponent: React.FC<{ item: FAQContentItem }> = ({ item }) => {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <div
      className={`group transition-all duration-500 rounded-[2rem] border border-gray-100 dark:border-gray-800 bg-white dark:bg-slate-900/50 p-8 hover:shadow-2xl hover:border-primary/30 ${
        isOpen ? 'ring-2 ring-primary/20 bg-primary/[0.02] dark:bg-primary/[0.05]' : 'shadow-sm'
      }`}
    >
      <button
      ...
      >
        ...
        <div
          className={`shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${
            isOpen ? 'bg-primary text-white rotate-180' : 'bg-gray-50 dark:bg-slate-800 text-primary'
          }`}
        >
          <span className="material-symbols-outlined !text-2xl">{isOpen ? 'remove' : 'add'}</span>
        </div>
      </button>
...
export const FAQSection: React.FC<{ items: FAQContentItem[] }> = React.memo(({ items }) => (
